...
原本是一套「V6.0 米诺提示词」放在 Claude Code 里给人用：**用户跟 AI 对话 → AI 理解用户 → 产出报告**。V3 就是把这套原样搬进小程序。

**只有两大块**：
- **米诺对话**：唯一的核心交互引擎。
- **报告库**：唯一的交付物。报告 = LLM 把对话总结成的 Markdown 文档，也是「越来越懂用户」的沉淀。

**唯一动线**：收到「今日一问」推送 → 跟米诺聊透 → 点「让米诺写份报告」或米诺主动写 → 报告进库 + 微信通知 → 回来接着聊，下次米诺更懂你。**对话产出报告，报告喂养对话。**
...
FastGPT（引擎，产品灵魂）
  ├─ 应用：装 V6.0 提示词 = 米诺人格（轻量「势/节奏」比喻叙事，**不排盘**）
...
> **关键**：没有八字排盘引擎，没有评分公式，不让 LLM 生成任何精确数字。米诺只做「对话 + 写 Markdown」。
...
│   ├─ 右上「连续 N」            = streak 芯片
│   ├─ 流式对话                 直连 FastGPT
│   ├─ 对话后 suggestions        每轮回复末尾长出「接着聊」气泡，其一是「让米诺写份报告」
│   └─ 报告生成弹层             点「生成报告」/米诺主动写 → loading → 进库 + 推送
└─ 报告库 tab（交付物 = 知识库）
    ├─ 报告列表                 入口=tab根；顶部 4 类筛选 chip
    ├─ 报告详情·战略分析         Markdown：主要矛盾 / 定位 / 三步走 + 米诺批注
    ├─ 报告详情·创业履历         自传体叙事正文 + 米诺批注 + 溯源
    ├─ 报告详情·复盘战报 / 决策记录  同为 Markdown 文档，版式复用
    └─ 每份报告：可读 / 分享 / 导出 / 「跟米诺聊这份报告」回对话
```
...
| type | 内容 | 触发 |
|---|---|---|
| `strategy` 战略分析 | 主要矛盾 / 定位 / 三步走 | 用户点生成 / 米诺主动 |
| `resume` 创业履历 | 个人履历·创业故事·自传体叙事 | 用户点生成 / 米诺主动 |
| `review` 复盘战报 | 阶段性复盘、这一周/这一阶段的战报 | 用户点生成 / 定时提醒后 |
| `decision` 决策记录 | 一个决定的理由/预期/待验证 | 用户点生成 / 对话中提炼 |

**两种触发**（都要）：
1. **用户按需**：对话里点 suggestion「让米诺写份报告」→ 弹生成弹层 → 后端调 FastGPT「总结成报告」工作流 → 存库 → 微信通知。
2. **米诺主动**：对话达成某标记（聊透一件事 / 到复盘时点）时，后端追加一次生成调用，主动产出并推送「米诺刚写好一份…」。

**生成流程（技术）**：
```
用户对话若干轮（FastGPT，要点已写入知识库）
  → 触发「生成报告」：后端带 {conversation要点 + 用户知识库检索} 调 FastGPT 报告工作流
  → 返回 Markdown（title + body + 米诺批注 annotation + 关联对话 source）
...
- **今日一问（任务派发）**：cron 每天生成一条 `tasks` 记录（米诺的一个引导问题），渲染成对话页顶部卡；点「开始聊」把该问题作为对话开场（`prompt_seed`）。
- **随机「米诺突然找你」**：后台按沉默天数随机推一条带悬念的邀约（可选，二期）。
...
reports                                       # 核心交付物 = Markdown 文档
  ...
  title, body_md(text), annotation(text),     # body_md=正文, annotation=米诺批注
  source_conversation_id(FK,null),            # 溯源
  origin('user'|'agent'),                     # 用户点生成 / 米诺主动
...
POST /reports/:id/append     「跟米诺补充」→ 回对话续写后更新报告
...
**人格权威来源**：项目根目录的系统提示词文档 `⽶诺战略参谋部 · 天势终极版 V6.0`。把其中【哲学内核「势」/ 五种角色语气 / 术语翻译成比喻 / 禁用词 / 鼓励用词 / 埋悬念钩子 / 十二问 / 复盘流程】作为 FastGPT 应用的 system prompt。`米诺对话（可对话）.dc.html` 内含一份浓缩可用版可直接取用。
...
3. 建一个**「总结成报告」的工作流/提示**：输入=对话内容+知识库检索结果，输出=对应 type 的 Markdown（含米诺批注）。后端 `/reports/generate` 调它。

**上下文拼装**（保证因人而异、跨对话记忆）：
```
system = 米诺人格（势哲学/语气铁律/输出口径，禁用赋能·抓手·底层逻辑等词）
...
底部 tab：高 60–64px，2 tab（米诺/报告库），激活 #B23A2E，未激活 #9B9384。
Markdown 报告版式：小节前加 4px 竖色条 + 衬线小标题；米诺批注用左描金竖线引用块。
```
...
- `米诺对话（可对话）.dc.html` — 可真实对话的米诺页（含内置 system prompt、指令栏）。**LLM 行为参考**。
...
6. **打磨**：报告分享/导出、「跟米诺补充/聊这份报告」回流、随机邀约（可选）。
